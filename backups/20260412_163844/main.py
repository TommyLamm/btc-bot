"""
BTC-Bot 主程序（自適應進化版 v3.4 — 穩定交易版）
修復：
  1. 開倉只在 HL 確認成交後才更新本地狀態
  2. 平倉前 HL executor 會同步實際倉位
  3. 平倉失敗自動重置，避免卡在假倉位
"""
...
HL_LEVERAGE = int(os.environ.get("HL_LEVERAGE", "2"))
HL_POSITION_SIZE_USD = float(os.environ.get("HL_POSITION_SIZE_USD", "10.0"))

TARGET_TRADE_PCT = float(os.environ.get("TARGET_TRADE_PCT", "0.08"))
DYNAMIC_THRESHOLD_WARMUP = int(os.environ.get("DYNAMIC_THRESHOLD_WARMUP", "20"))
INITIAL_THRESHOLD = float(os.environ.get("INITIAL_THRESHOLD", "0.15"))
...
class BTCBot:
    def __init__(self):
        ...
        self.monitor.initial_threshold = INITIAL_THRESHOLD
        ...
        if HL_ENABLED:
            try:
                ...
                self.hl_executor = HyperliquidExecutor(
                    ...
                    position_size_usd=HL_POSITION_SIZE_USD,
                    coin="BTC",
                    log_path="logs/trades.jsonl",
                )
                print(f"\n  [HL] 交易模組已啟用")
                print(f"  [HL] {self.hl_executor.status_summary()}")
    ...
    def _run_adaptive_search(self):
        ...
        if result:
            ...
            print(f"  初始閾值: {INITIAL_THRESHOLD} → {DYNAMIC_THRESHOLD_WARMUP} 根K線後動態")
    ...
    def _hl_open(self, direction, price, signal):
        if not self.hl_executor:
            return None
        reason = f"signal={signal:+.3f} regime={self.current_regime} price={price:.1f}"
        try:
            if direction == "long":
            ...
            if result["status"] == "ok":
                print(f"  [HL] 實盤開倉: {result['direction']} {result['size']} BTC @ ${result['price']:,.1f}")
    ...
    def _hl_close(self, reason=""):
        ...
        try:
            ...
            if result["status"] == "ok":
                print(f"  [HL] 實盤平倉: {result['direction']} @ ${result['price']:,.1f} PnL=${result['pnl']:+.4f}")
            elif result["status"] != "skip":
    ...
    async def on_candle_close(self, candle, buffer):
        ...
        if SIGNAL_DEBUG and self.candle_count % 3 == 0:
            ...
            if thr_source == "dynamic":
                ...
                thr_info = f"init={threshold:.4f} ({n_realtime}/{DYNAMIC_THRESHOLD_WARMUP})"
            print(f"  [信號] sig={signal:+.4f} {thr_info} → {action_str}")
        ...
        if self.position is not None:
            if (self.position == "long" and action != "long") or \
               (self.position == "short" and action != "short"):
                ...
                self._hl_close(f"signal_exit pnl={pnl:+.4f}")
                self.position = None
                self.entry_price = 0.0
                traded = True
        ...
        if action and self.position is None:
            open_result = self._hl_open(action, candle["close"], signal)
            if open_result and open_result.get("status") == "ok":
                self.position = action
                direction = "做多" if action == "long" else "做空"
                print(f"  ★ 開倉 {direction} @ {candle['close']:.1f} 信號={signal:+.3f} 閾值={threshold:.3f} [{self.current_regime}]")
    ...
    async def run(self):
        print("\n" + "=" * 60)
        print("BTC-Bot 啟動（自適應進化版 v3.4 — 穩定交易版）")
        print(f"  基礎因子: 12 個")
        ...
        if self.hl_executor:
            print(f"  交易模式: Hyperliquid 實盤 ({HL_LEVERAGE}x 槓桿, ${HL_POSITION_SIZE_USD} 保證金)")
            print(f"  {self.hl_executor.status_summary()}")
        else:
            print(f"  交易模式: 純信號（未啟用實盤）")
        print(f"  閾值策略: 初始={INITIAL_THRESHOLD} → {DYNAMIC_THRESHOLD_WARMUP}根K線後動態 (目標{TARGET_TRADE_PCT*100:.0f}%)")
        print(f"  預期交易: 每天 {int(288*TARGET_TRADE_PCT)}~{int(288*TARGET_TRADE_PCT*1.5)} 次")
