"""
Hyperliquid 永續合約交易執行器 v2
修復：
  1. 平倉前同步 HL 實際倉位，避免 "asset=0" 錯誤
  2. 開倉前也同步倉位，避免重複開倉
  3. 更好的錯誤處理和日誌
"""
...
class HyperliquidExecutor:
    ...
    def open_long(self, reason: str = "") -> dict:
        return self._open_position(True, reason)

    def open_short(self, reason: str = "") -> dict:
        return self._open_position(False, reason)

    def _open_position(self, is_buy: bool, reason: str = "") -> dict:
    ...
    def _log_trade(self, action, direction, price, size, reason="", pnl=None):
        record = {
    ...
    def status_summary(self) -> str:
        ...
        return (
            ...
            f"累計PnL=${self.total_pnl:+.4f} | 交易={self.trade_count}筆"
        )
