"""
BTC-Bot 遺傳編程（GP）因子挖掘器
自動從基礎因子中進化出新的複合因子表達式。
"""
...
UNARY_OPS = ["abs", "neg", "square", "sign", "log1p"]
...
class GPNode:
    ...
    def to_string(self):
        if self.node_type == "factor":
            ...
            op_map = {"add": "+", "sub": "-", "mul": "*", "div": "/", "max": "max", "min": "min"}
...
class GPFactorMiner:
    def __init__(self, pop_size=200, n_generations=40, max_depth=4,
                 tournament_size=5, crossover_rate=0.6, mutation_rate=0.3,
                 ic_threshold=0.02, max_new_factors=5, save_path="data/gp_factors.json"):
            ...
            try:
                ...
                self.discovered_factors = [
                    {"name": d["name"], "expr": d["expr"],
                     "tree": GPNode.from_dict(d["tree"]), "ic": d["ic"]}
                    for d in data
                ]
    ...
    def _random_tree(self, depth=0):
        if depth >= self.max_depth or (depth > 1 and random.random() < 0.3):
            if random.random() < 0.85:
                return GPNode("factor", random.choice(BASE_FACTORS))
            else:
    ...
    def mine(self, data, returns):
        print(f"\n  GP 因子挖掘開始（種群={self.pop_size}, 代數={self.n_generations}）")
        ...
        for gen in range(self.n_generations):
            ...
            for tree in population:
                ...
                scored.append((tree, ic))
            scored.sort(key=lambda x: x[1], reverse=True)

            if (gen + 1) % 10 == 0:
                print(f"    第 {gen+1:>3d} 代 | 最佳 IC={scored[0][1]:.4f} | {scored[0][0].to_string()[:50]}")

            for tree, ic in scored[:10]:
                expr = tree.to_string()
                if ic >= self.ic_threshold and expr not in seen_exprs:
                    if tree.node_type != "factor" and tree.size() > 1:
                        seen_exprs.add(expr)
                        best_factors.append({"tree": tree, "expr": expr, "ic": ic})
        ...
        for f in best_factors:
            if f["expr"] not in unique or f["ic"] > unique[f["expr"]]["ic"]:
                unique[f["expr"]] = f
        new_factors = sorted(unique.values(), key=lambda x: x["ic"], reverse=True)[:self.max_new_factors]

        timestamp = datetime.now().strftime("%m%d%H%M")
        result = []
        for i, f in enumerate(new_factors):
            name = f"gp_{timestamp}_{i}"
            factor = {"name": name, "expr": f["expr"], "tree": f["tree"], "ic": f["ic"]}
            result.append(factor)
            self.discovered_factors.append(factor)
            print(f"    新因子: {name} | IC={f['ic']:.4f} | {f['expr'][:60]}")

        self.discovered_factors.sort(key=lambda x: x["ic"], reverse=True)
        self.discovered_factors = self.discovered_factors[:20]
        self._save_factors()
    ...
    def get_top_factors(self, n=5):
        sorted_f = sorted(self.discovered_factors, key=lambda x: x["ic"], reverse=True)
