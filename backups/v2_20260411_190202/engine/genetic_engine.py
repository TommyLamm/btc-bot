...
class GeneticEngine:
    def __init__(self, pop_size=300, n_generations=60, train_ratio=0.7,
                 mutation_rate=0.15, crossover_rate=0.6, threshold_range=(0.3, 2.0)):
    ...
    def _evaluate(self, individual, X, ret):
        ...
        mask = signal != 0
        if mask.sum() < 30:
            return {"win_rate": 0.0, "n_trades": 0, "profit_factor": 0.0, "sharpe": 0.0}
        trade_returns = trades[mask]
        ...
        sharpe = float(trade_returns.mean() / (trade_returns.std() + 1e-10) * np.sqrt(252 * 288))
        return {"win_rate": win_rate, "n_trades": n_trades, "profit_factor": profit_factor, "sharpe": sharpe}

    def _fitness(self, metrics):
        wr, nt, pf = metrics["win_rate"], metrics["n_trades"], metrics["profit_factor"]
        if nt < 30:
            return -999.0
        return wr * 100 + min(nt / 100, 3.0) + min(pf, 2.0) * 2 - max(0, 30 - nt) * 0.5
    ...
    def _crossover(self, parent1, parent2):
        ...
        return {"weights": child_weights, "threshold": random.choice([parent1["threshold"], parent2["threshold"]])}
    ...
    def search(self, df):
        ...
        if best_individual is not None:
            ...
            print(f"\n  驗證集: 勝率={vm['win_rate']:.3f} 交易={vm['n_trades']} PF={vm['profit_factor']:.2f}")
